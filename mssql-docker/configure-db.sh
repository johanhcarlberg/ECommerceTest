...
if [ -n "$DATABASE" ]; then
    ...
    /opt/mssql-tools/bin/sqlcmd -S localhost -U sa -P $SA_PASSWORD -d master -q "SELECT CASE WHEN EXISTS (SELECT count(*) as 'Database created' FROM sys.Databases WHERE name = '${DATABASE}') THEN 'TRUE' ELSE 'FALSE' END as 'Database created';"
